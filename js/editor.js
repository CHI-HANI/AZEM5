...
// الخطوات: lang → auth → password-link → info → done
// navigation يدوي عبر obGoToStep
const OB_STEPS = ['lang','auth','password-link','info','done'];
...
function obSelectLang(code) {
  ...
  S.lang = code;
  applyLang(code);
  setTimeout(() => obGoToStep('auth'), 300);
}

function obNext() {
  // للخطوات التي تملك زر "التالي"
  if (obCurrentStep === 'info') {
    // تحقق من الوزن والطول
    ...
    // إرسال فوري إلى Sheets إذا كان المستخدم مسجلاً
    if (window._fbUser && typeof sendToSheets === 'function') {
      sendToSheets(window._fbUser, {});
    }
    obFinish();
    return;
  }
  ...
  // ── Auth ─────────────────────────────────
    ...
    html = `<div style="min-height:100vh;display:flex;flex-direction:column;justify-content:center;padding:32px 24px;gap:16px;">
      ...
      <input id="ob-pass-inp" type="password" placeholder="${_T3('كلمة المرور (6 أحرف+)','Password (6+ chars)','Mot de passe (6+ car.)')}"
        style="width:100%;padding:14px 16px;border-radius:14px;background:var(--card);border:1.5px solid var(--border);color:var(--txt);font-family:'Cairo',sans-serif;font-size:14px;box-sizing:border-box;outline:none;direction:ltr;text-align:left;"
        oninput="document.getElementById('ob-auth-err').style.display='none'">
      <input id="ob-name-inp" type="text" placeholder="${_T3('اسمك','Your name','Votre nom')}"
        style="width:100%;padding:14px 16px;border-radius:14px;background:var(--card);border:1.5px solid var(--border);color:var(--txt);font-family:'Cairo',sans-serif;font-size:14px;box-sizing:border-box;outline:none;">

      <!-- Buttons -->
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <button id="ob-email-btn" onclick="obFirebaseEmailAuth('signup')" style="padding:14px;border-radius:14px;background:linear-gradient(135deg,var(--gl),var(--gd));color:var(--night);font-family:'Cairo',sans-serif;font-size:14px;font-weight:900;cursor:pointer;border:none;-webkit-appearance:none;appearance:none;">
          ${_T3('إنشاء حساب','Sign Up','S\'inscrire')}
        </button>
        <button onclick="obFirebaseEmailAuth('login')" style="padding:14px;border-radius:14px;background:var(--card);border:1.5px solid var(--border);color:var(--txt);font-family:'Cairo',sans-serif;font-size:14px;font-weight:700;cursor:pointer;-webkit-appearance:none;appearance:none;">
