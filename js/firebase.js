...
// ══════════════════════════════════════════
// Webhook — إرسال فوري إلى Google Sheets
// ══════════════════════════════════════════
  ...
  try {
    ...
    if (isStandalone) {
      ...
      if (result && result.user) {
        ...
        // هل توجد بيانات في Firestore؟
        const hasData = await pullFromCloud(result.user.uid);
        if (hasData) {
          // مستخدم قديم — اذهب مباشرة للتطبيق
          obFinish();
        } else {
          // مستخدم جديد — اعرض خيار كلمة المرور ثم المعلومات
...
getRedirectResult(auth).then(async result => {
  if (result && result.user) {
    ...
    if (wasOb) {
      // came back from onboarding Google redirect
      window._obGoogleJustSignedIn = true;
      if (result.user.displayName) {
        S.user = S.user || {};
        S.user.name = result.user.displayName.split(' ')[0];
