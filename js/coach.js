...
- **ممنوع منعاً باتاً تكرار نفس الجملة أكثر من مرة في الرد — إذا لم يكن لديك معلومات كافية قل ذلك مرة واحدة فقط وأنهِ الرد**
- يمكنك مناقشة أي موضوع خارج اللياقة إذا طلب المستخدم ذلك
...
function deRenderLibGrid() {
  ...
  grid.innerHTML = filtered.map(ex => {
    ...
    return `<div onclick="deLibAddEx('${ex.id}')" style="border-radius:16px;background:var(--card);border:2px solid ${borderColor};overflow:hidden;cursor:pointer;position:relative;-webkit-mask-image:-webkit-radial-gradient(white,black);">`
      + `<div style="width:100%;aspect-ratio:1/1;background:${bgTint};position:relative;overflow:hidden;-webkit-mask-image:-webkit-radial-gradient(white,black);">`
